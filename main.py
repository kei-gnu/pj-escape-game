...
def print_out(map):
    ...
    if goal_open == 0:
        ...
        print_map[goal_pos[0]][goal_pos[1]] = "□"
    ...
    for mapline in print_map:
        ...
        mapline = [f"{Color.WHITE}■ {Color.RESET}" if i == "1" else i for i in mapline]
...
def check_game_over(start_time):
    ...
    time_rem = timeLimit-round(limit)
    if limit > timeLimit:
        limit == timeLimit 
    if limit < timeLimit:
        if time_rem > 50:
            print(f"爆発まであと{time_rem}秒")
        else:
            print(f"爆発まであと{Color.RED}{time_rem}{Color.RESET}秒")
    elif limit >= timeLimit:
        return 1
...
def print_rule():
    ...
    print(f"X君…{Color.RED}♥{Color.RESET}　Y君…{Color.BLUE}♥{Color.RESET}　アイテム…☆　スイッチ…〇　ドア…□")
            ...
            if find_obj.item_info not in play_player.item_have:#必要なアイテムを持っていない
                ...
                if find_obj == passward_4_box or find_obj == projector_box or find_obj == passward_6_box:#パスワードだった場合、相方にもアイテムを渡す
                    ...
                    if find_obj == righter_oil_box:
                        ...
                        play_player.item_have.remove("カッター")
                        find_obj.empty = 1
        ...
        if find_obj.door_num == 0:#ifアイテムが必要なドアである
            if find_obj.door_item_info in play_player.item_have:#ifそのアイテムを持っている
                ...
                #        return "パスワードが間違っています"
                    ...
                    find_obj.op_cl = 0
                    play_player.item_have.remove(find_obj.door_item_info)
